...
Elm.Soccer = Elm.Soccer || {};
Elm.Soccer.make = function (_elm) {
   "use strict";
   _elm.Soccer = _elm.Soccer || {};
   if (_elm.Soccer.values) return _elm.Soccer.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var renderGroups = function (groups) {
      ...
      function (element) {
         return A2($Html.li,
         _U.list([]),
         _U.list([$Html.text(element)]));
      },
      groups));
   };
   var identify = F2(function (element,list) {
      return A2($List.member,
      element,
      list) ? $Maybe.Just(element) : $Maybe.Nothing;
   });
   var TeamClicked = function (a) {
      return {ctor: "TeamClicked",_0: a};
   };
   var teamItem = F2(function (address,team) {
      return A2($Html.li,
      _U.list([A2($Html$Events.onDoubleClick,
              ...
              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                              ...
                                              ,{ctor: "_Tuple2",_0: "-o-user-select",_1: "none"}]))]),
      _U.list([$Html.text(team.name)]));
   });
   var EnterGroupName = function (a) {
      return {ctor: "EnterGroupName",_0: a};
   };
   var EnterTeamName = function (a) {
      return {ctor: "EnterTeamName",_0: a};
   };
   var RemoveTeam = function (a) {
      return {ctor: "RemoveTeam",_0: a};
   };
   var AddTeam = F2(function (a,b) {
      return {ctor: "AddTeam",_0: a,_1: b};
   });
   var view = F2(function (address,model) {
      var distinctGroups = $List$Extra.unique(A2($List.map,
      function (_) {
         return _.group;
      },
      model.teams));
      return A2($Html.div,
      _U.list([]),
      _U.list([renderGroups(distinctGroups)
              ,A2($Html.div,
              ...
              _U.list([A2($Html.ul,
              _U.list([]),
              A2($List.map,teamItem(address),model.teams))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                       ,_0: "border"
                                                       ,_1: "1px solid black"}
              ...
              _U.list([A2($Html.input,
                      _U.list([$Html$Attributes.placeholder("Group...")
                              ,A3($Html$Events.on,
                              "input",
                              $Html$Events.targetValue,
                              function (_p1) {
                                 return A2($Signal.message,address,EnterGroupName(_p1));
                              })
                              ,$Html$Attributes.value(model.newGroupName)
                      ...
                      _U.list([]))
                      ,A2($Html.input,
                      _U.list([$Html$Attributes.placeholder("Team name...")
                              ,A3($Html$Events.on,
                              "input",
                              $Html$Events.targetValue,
                              function (_p2) {
                                 return A2($Signal.message,address,EnterTeamName(_p2));
                              })
                              ,$Html$Attributes.value(model.newTeamName)
                              ,A2(onEnter,
                              address,
                              A2(AddTeam,model.newTeamName,model.newGroupName))
                              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                                               ,_0: "margin-right"
                                                               ,_1: "2px"}]))]),
                      _U.list([]))
   ...
   var newTeam = F3(function (idInput,nameInput,groupInput) {
      return {id: idInput
             ,name: nameInput
             ,group: groupInput
             ,selected: false};
   ...
   var update = F2(function (action,model) {
      var _p3 = action;
      switch (_p3.ctor)
      {case "TeamClicked": return _U.update(model,
           {teams: A2($List.map,
           function (t) {
              return _U.eq(t.id,_p3._0) ? _U.update(t,
              {selected: $Basics.not(t.selected)}) : _U.update(t,
              {selected: t.selected});
           },
           model.teams)
           ,teamsSelected: A2($List.filterMap,
           function (t) {
              return _U.eq(t.selected,
              true) ? $Maybe.Just(t.id) : $Maybe.Nothing;
           },
           model.teams)});
         case "EnterTeamName": return _U.update(model,
           {newTeamName: _p3._0});
         case "EnterGroupName": return _U.update(model,
           {newGroupName: _p3._0});
         case "AddTeam": var _p4 = _p3._0;
           return _U.update(model,
           {teamId: model.teamId + 1
           ,teams: $String.isEmpty(_p4) ? model.teams : A2($Basics._op["++"],
           model.teams,
           _U.list([A3(newTeam,model.teamId,_p4,_p3._1)]))
           ,newTeamName: ""
           ,newGroupName: ""});
         default: return _U.update(model,
           {teams: A2($List.filter,
           function (team) {
              return !_U.eq(team.id,_p3._0);
           },
           model.teams)});}
   });
   var Team = F4(function (a,b,c,d) {
      return {id: a,name: b,group: c,selected: d};
   });
   var emptyModel = {teams: _U.list([{id: 1
                                     ,name: "test"
                                     ,group: "A"
                                     ,selected: false}])
                    ,teamId: 0
   ...
   var main = $StartApp$Simple.start({model: emptyModel
                                     ,view: view
                                     ,update: update});
   return _elm.Soccer.values = {_op: _op
                               ,main: main
                               ,Model: Model
